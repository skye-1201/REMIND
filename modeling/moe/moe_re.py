...
class Expert(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 aug_mode='noise', aug_ratio=0.1, aug_noise_std=0.02):
        super().__init__()
        ...
        self.aug = PerturbationAug(mode=aug_mode, ratio=aug_ratio, noise_std=aug_noise_std)
...
class ExpertBranch(nn.Module):
    def __init__(self, input_dim, num_experts=4, mlp_ratio=4.0, drop=0.0,
                 aug_mode='noise', aug_ratio=0.1, aug_noise_std=0.02):
        super().__init__()
        self.num_experts = num_experts
        mlp_hidden_dim = int(input_dim * float(mlp_ratio))
